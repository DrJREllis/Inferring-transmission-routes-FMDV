function []=ModelSelection(testname,farm,Nrounds,Nsuc,Nw)
...
[ParVp, ParTr,~]=LoadParameters(0); %Load data for prior distributions 
Par=[ParVp ParTr]; %Combine parameters into one matrix
[~,dpar,pardis]=priordistfitting(Par,0,'NA','NA',Model); % Fits the priors to a distribution
...
parfor n=1:Nw % Split each round between  workers - change to for loop if parellel computing is not available
    ...
    while i<=ceil(Nsuc/Nw) % The number of success from each worker
        ...
        [VP, Ti, ~]=ViralProfile(beta(1:11),N,Maxdays,dt); %Create the viral profile of all animals 
        if Maxdays>ceil(Ti(1,2))+Age+1
          %Set the number of days depending on how long for clinical signs to appear:
            ...
            if Model==1
                [~,Cs,Cfirst,~,~,~,~,~,~]=Transmission_model(N,Ndays,dt,beta(12:21),Ni00,E0,VP,Ti,Esize,EnvTrType); %Run the transmission model
            else if Model==2
                    [Cs,Cfirst,~,~,~,~,~,~]=Direct_model(N,Ndays,dt,beta(12),Ni00,VP,Ti);
                else if Model==3
                        [~,Cs,Cfirst,~,~,~,~]=Env_model(N,Ndays,dt,beta(13:21),Ni00,E0,VP,Ti,Esize,EnvTrType);
...
parfor n=1:Nw % Split each round between  workers - change to for loop if parellel computing is not available
    ...
    %Sample particle for the selected model from previous round with weights and add the perturbation  
        ...
        [PriorProb,~,~]=priordistfitting(Par,beta,dpar,pardis,Model);
    ...
    % Restrict parameters where applicable by generating a new perturbation (redo the above)
        ...
        if Maxdays>ceil(Ti(1,2))+Data_age(farm)+1
            ...
            %Run the chosen transmission model:
            if Model==1
                [~,Csp{n}{i},Cfirst{n}(i),~,~,~,~,~,~]=Transmission_model(N,Ndays,dt,beta(12:21),Ni00,E0,VP,Ti,Esize,EnvTrType); 
            else if Model==2
                    [Csp{n}{i},Cfirst{n}(i),~,~,~,~,~,~]=Direct_model(N,Ndays,dt,beta(12),Ni00,VP,Ti);
                else if Model==3
                        [~,Csp{n}{i},Cfirst{n}(i),~,~,~,~]=Env_model(N,Ndays,dt,beta(13:21),Ni00,E0,VP,Ti,Esize,EnvTrType);
                    else print('Model number error')
                    end
                end
            end
