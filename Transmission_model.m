function [E,Cs,Cfirst,Ni,TotInf,dailyI,InfCause,Infectiousness,ProbE,ProbD]=Transmission_model(N,Ndays,dt,ParTr,Ni0,E0,VP,Ti,Esize)
...
Ni=zeros(Nt,1); V=zeros(N,Nt); Infectiousness=zeros(N,Nt+1); State=zeros(N,Nt); E=zeros(Nt+1,4);
...
for j=1:Nt
    ...
    if Ni(j)<N %If there are still uninfected animals in the herd
        ...
        InfCause(S,2)=j*dt;
        State(S,j+1)=State(S,j)+max(Einf,Dinf); 
